...
async function getData(): Promise<User[]> {
  ...
  return apiData.map((item: any) => ({
    ...
    country: item.Country // Added country field
  }));
}
  ...
  const handleAction = async (email: string, status: number, reason?: string) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/admin/ChangeAgentApprovalStatus/${email}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      <Dialog open={rejectionDialogOpen} onOpenChange={setRejectionDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to reject this agent? Please provide a reason.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-3 md:col-span-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <DetailItem 
                    ...
                    value={selectedUser?.COI_Certificate} 
                    isFile={true} 
                  />
                </div>
              </div>
